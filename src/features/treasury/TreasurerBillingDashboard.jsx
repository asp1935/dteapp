...
const TreasurerBillingDashboard = () => {
  ...
  const { bills, selectedBill, selectedBillApprovals, loading, fetching, success, error } = useSelector((state) => state.billing);
  const { institutions } = useSelector((state) => state.institutions);

  const [filterInst, setFilterInst] = useState('');
  const [filterStatus, setFilterStatus] = useState('SUBMITTED_TO_TREASURY');
  ...
  const columns = [
    ...
    { key: 'bill_status', label: 'Status', render: (val) => (
      <span className={cn(
        ...
        val === 'SUBMITTED_TO_TREASURY' ? "bg-indigo-100 text-indigo-600" : val === 'PAID' ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-500"
      )}>
        {val?.replace('_', ' ')}
      </span>
    )}
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
        <div className="flex flex-wrap gap-6 items-end">
          ...
          <div className="flex-1 min-w-[200px]">
             ...
             >
               <option value="SUBMITTED_TO_TREASURY">Awaiting Disbursement</option>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-1 shadow-sm overflow-hidden">
        {fetching ? (
          ...
          <Table 
            ...
            data={bills} 
            actions={(row) => (
              <Button variant="ghost" onClick={() => handleViewDetails(row.id)} className="text-emerald-600 hover:bg-emerald-50 font-black text-[10px] uppercase tracking-widest">
                <Eye size={16} className="mr-2" /> DISBURSE
              </Button>
            )}
          />
        )}
          ...
          <div className="space-y-8 p-1">
             ...
             <div className="space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-3xl p-6 space-y-4">
                   {selectedBillApprovals.map((app, idx) => (
                     <div key={idx} className="flex items-start gap-4">
                        <div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0">
                           <CheckCircle2 size={12} />
                        </div>
                        <div className="flex-1">
                           <p className="text-xs font-bold text-slate-900">{app.level.replace('_', ' ')} Verified</p>
             ...
             {/* Treasury Action */}
             {selectedBill.bill_status === 'SUBMITTED_TO_TREASURY' && (
