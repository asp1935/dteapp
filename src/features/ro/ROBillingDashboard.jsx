...
const ROBillingDashboard = () => {
  ...
  const [filterStatus, setFilterStatus] = useState('SUBMITTED_TO_RO');
  ...
  const columns = [
    ...
    { key: 'bill_status', label: 'Status', render: (val) => (
      <span className={cn(
        ...
        val === 'SUBMITTED_TO_RO' ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"
      )}>
        {val?.replace('_', ' ')}
      </span>
    )}
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
        <div className="flex flex-wrap gap-6 items-end">
          ...
          <div className="flex-1 min-w-[200px]">
             ...
             >
               <option value="SUBMITTED_TO_RO">Pending My Review</option>
               <option value="SUBMITTED_TO_TREASURY">Sent to Treasury</option>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-1 shadow-sm overflow-hidden">
        {fetching ? (
          ...
          <Table 
            ...
            data={bills} 
            actions={(row) => (
              <Button variant="ghost" onClick={() => handleViewDetails(row.id)} className="text-indigo-600 hover:bg-indigo-50 font-black text-[10px] uppercase tracking-widest">
      ...
      >
        {selectedBill && (
          <div className="space-y-8 p-1">
             ...
             {/* RO Action Area */}
             {selectedBill.bill_status === 'SUBMITTED_TO_RO' && (
