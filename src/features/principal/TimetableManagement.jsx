...
const TimetableManagement = () => {
  ...
  const addSlot = (day) => {
    const daySlots = slots.filter(s => (s.day || s.day_of_week) === day);
    const nextSlotNumber = Math.min(daySlots.length + 1, 8);
  ...
  const handleSaveBulk = async () => {
    ...
    const payload = {
      ...
      academic_year: selectedYear,
      slots: slots.map(({ id, isNew, day, ...rest }) => ({
        ...rest,
        day_of_week: rest.day_of_week || day
      }))
    };
          ...
          <div key={day} className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm transition-all hover:shadow-md">
            ...
            <div className="p-8">
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {slots.filter(s => (s.day || s.day_of_week) === day).map(slot => (
                 ...
                 {slots.filter(s => (s.day || s.day_of_week) === day).length === 0 && (
